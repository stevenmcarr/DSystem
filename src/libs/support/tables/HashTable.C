...
HashTable::HashTable ()
{
  ...
  HashFunctCallback    = (HashFunctFunctPtr)NULL;
  RehashFunctCallback  = (RehashFunctFunctPtr)NULL;
  EntryCompareCallback = (EntryCompareFunctPtr)NULL;
  EntryCleanupCallback = (EntryCleanupFunctPtr)NULL;
   ...
   if (hashTableCreated)
     {
       ...
       while (nextSlot != FIRST_SLOT)
         {
            HashTable::DeleteEntry ((void*)cEntries);
         }
       ...
       if (indexSet[index] < 0)
         ...
         { // entry already present
            if (AddEntryCallback != NULL)
              ...
              {
                 ChooseEntryCleanup ((void*)&cEntries[entrySize * indexSet[index]]);
                 memcpy (&cEntries[entrySize * indexSet[index]], entry, entrySize);
              }
         }
    }
            ...
            if (last >= 0)
              {
                ...
                if (DeleteEntryCallback != NULL)
                  ...
                  {
                    ChooseEntryCleanup ((void*)&cEntries[last * entrySize]);
                  }
              ...
              { // there is only one entry left in the table
                ...
                if (DeleteEntryCallback != NULL)
                  ...
                  {
                    ChooseEntryCleanup ((void*)&cEntries[0]);
                  }
...
{
  return DefaultHashFunct (entry, size);
}
...
{
  return DefaultRehashFunct (oldHashValue, size);
}
...
{
  return DefaultEntryCompare (entry1, entry2);
}
...
//
//
int HashTable::QueryIndexSet (const void* entry, const Boolean addingEntry) const
{
  ...
  currentIndex = initialIndex = This->ChooseHashFunct(entry, indexSetSize);
       ...
       switch (indexSet[currentIndex])
         {
            ...
            default:
              if (This->ChooseEntryCompare(entry, 
...
#      endif
       ...
       currentIndex = This->ChooseRehashFunct(currentIndex, indexSetSize);
       rehashCount ++;
...
{
  register int i, initialIndex, finalIndex, size;
  char* cEntries = (char*)entries;
...
# endif
  ...
  for (i = 0; i < nextSlot; i++)
    {
       finalIndex = initialIndex = ChooseHashFunct (&cEntries[entrySize * i], indexSetSize);
...
#           endif
            ...
            finalIndex = ChooseRehashFunct (finalIndex, indexSetSize);
...
//
//
uint HashTable::ChooseHashFunct (const void* entry, const uint size)
{
  if (HashFunctCallback) return HashFunctCallback (entry, size);
  else                   return HashFunct (entry, size);
}

//
//
uint HashTable::ChooseRehashFunct (const uint oldHashValue, const int size)
{
  if (RehashFunctCallback) return RehashFunctCallback (oldHashValue, size);
  else                     return RehashFunct (oldHashValue, size);
}

//
//
int  HashTable::ChooseEntryCompare (const void* entry1, const void* entry2)
{
  if (EntryCompareCallback) return EntryCompareCallback (entry1, entry2);
  else                      return EntryCompare (entry1, entry2);
}

//
//
void HashTable::ChooseEntryCleanup (void* entry)
{
  if (EntryCleanupCallback) EntryCleanupCallback (entry);
  else                      EntryCleanup (entry);
...
{
  cerr << "\n\nError: Failure to call Create() before using the HashTable"
       << endl;
...
{
  cerr << "\n\nError: Failure to call Destroy() before deleting the HashTable"
       << endl;
...
static uint DefaultHashFunct (const void* entry, const uint size)
{
  cerr << "HashTable::DefaultHashFunct:\t" 
       << "Failure to specify HashFunct function.  Program terminated."
       << endl;
...
static int  DefaultEntryCompare (const void* entry1, const void* entry2)
{
  cerr << "HashTable::DefaultEntryCompare:\t"
       << "Failure to specify EntryCompare function.  Program terminated."
       << endl;
...
{
  if (currentEntryNumber < ((HashTable*)hashTable)->nextSlot)
    {
