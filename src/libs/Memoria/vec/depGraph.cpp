...
DependenceGraph::DependenceGraph(int V) 
{ 
	this->V = V; 
	adj = new std::list<int>[V]; 
	depEdges = new std::list<DG_Edge*>[V];
	stmts = new map<AST_INDEX,int>();
	sccs = new std::list<int>[V];
} 

int DependenceGraph::getIndex(AST_INDEX v) {
	map<AST_INDEX,int>::iterator sit;

	int	v_index = get_stmt_info_ptr(v)->stmt_num;
	if ((sit = stmts->find(v)) != stmts->end()) {
		stmts->insert(pair<AST_INDEX,int>(v,v_index));
	}
	return v_index;
}
