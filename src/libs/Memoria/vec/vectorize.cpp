...

{
	...
	if (level != LOOP_INDEPENDENT && dg[old_edge].type == dg_input)
	{
		...
		{
		case DDATA_GT:
			gen_put_dt_DIS(&dg[old_edge], level, DDATA_LT);
			temp = source;
			source = sink;
			sink = temp;
			break;
		case DDATA_GE:
			gen_put_dt_DIS(&dg[old_edge], level, DDATA_LE);
			temp = source;
			source = sink;
			sink = temp;
			break;
		default:
			if (dir > DDATA_BASE && dir < 0)
			{
			 ...
			 edge = next_edge)
		{
		  ...
		  can be modified */
				...
				for (i = 0; i < upd_info->num_loops; i++)
					...
					{
						dist = temp_vec[upd_info->loop_data[upd_info->heap[i].index].level - 1];
						if (dist < DDATA_BASE)
						...
						gen_put_dt_DIS(&dg[edge], i + 1, temp_vec[upd_info->loop_data[upd_info->heap[i].index].level - 1]);
		     ...
		     here */
				...
				while ((gen_get_dt_DIS(&dg[edge], i) == DDATA_ANY ||
					...
					{
						if ((nlevel = new_level(dg[edge], upd_info->num_loops, i + 1)) != LOOP_INDEPENDENT)
						{
							add_edge(upd_info->ped, &dg, edge, nlevel);
							i++;
						}
									  ...
									  arena_type *ar)
{
		...
		loop_data[loop].type == COMPLEX || loop_data[loop].type == TRAP)
	{
		...
		if (!loop_data[loop].stmts || loop_data[loop].inner_loop == -1 || levels_in_order(heap, loop_data, loop, loop_data[loop].level, outermost))
		{
