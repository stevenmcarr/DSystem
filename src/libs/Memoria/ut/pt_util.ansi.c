		  ...
		  Boolean *loop_normal_flag) /* Is this loop in normal form ? */
{
  ...
  *iter_cnt_node = pt_simplify_expr(*iter_cnt_node);
  iter_cnt_flag = !pt_eval(*iter_cnt_node, iter_cnt);
  
  /* Is loop in normal form ? */
  *loop_normal_flag = !lo_flag && (lo == 1) && !step_flag && (step == 1);
...
pt_perf_nest(AST_INDEX node)
{
    ...
    if (!is_do(node))	/* better start at a loop	*/
	return 0;
    
    while (true)	/* get to first real statement	*/
    {
	/* go to stmt in loop 	*/
	...
	if (!is_do(node) || (list_next(node) != AST_NIL))
	{
	    /* now in body of loop with > 1 stmts OR			*/
	    /*	body of loop where 1st stmt is not a do 		*/
	    
	    /* if any loops	found here then nesting not perfect	*/
	    
	    while (true)
	    {
		if (node == AST_NIL)	/* only stmts found		*/
		    return level;		/* return level found	*/
		
		if (is_do(node))		/* do found,		*/
		    return 0;			/* not perfect nest	*/
...
AST_INDEX
pt_next_inner_loop (AST_INDEX stmt)
{
    if (!(is_loop(stmt)))
	return AST_NIL;
    for (stmt = list_first(gen_DO_get_stmt_LIST(stmt)); 
